...
//  数据交互模块
//==========================================
define('_base/request',['smcore','_base/promise'],function(smcore) {
    ...
    new function() {
        ...
        for (var i = 0, axo; axo = s[i++];) {
            try {
                if (eval("new " + axo)) {
                    smcore.xhr = new Function("return new " + axo)
                    break;
                }
            } catch (e) {}
        }
    }
    var supportCors = "withCredentials" in smcore.xhr()
    ...
    function ajaxExtend(opts) {
        opts = smcore.mix({}, defaults, opts)
        opts.type = opts.type.toUpperCase()
        var querystring = typeof opts.data === "string" ? opts.data : smcore.param(opts.data)
...
var XHRMethods = {
            ...
            if (isSuccess) {
                if (status === 204) {
                    ...
                    if (typeof this.response === "undefined" && (this.responseText || this.responseXML)) {
                        ...
                        try {
                            this.response = smcore.ajaxConverters[dataType].call(this, this.responseText, this.responseXML)
                        } catch (e) {
                            isSuccess = false
                            this.error = e
    ...
    //ajax主函数
    smcore.ajax = function(opts, promise) {
        if (!opts || !opts.url) {
            smcore.error("参数必须为Object并且拥有url属性")
        }
        ...
        smcore.mix(promise, XHRProperties, XHRMethods)
        promise.then(opts.success, opts.error)
        "success error".replace(smcore.rword, function(name) { //绑定回调
            delete opts[name]
        })

        var dataType = opts.dataType //目标返回数据类型
        var transports = smcore.ajaxTransports
        ...
        smcore.mix(promise, transport) //取得传送器的request, respond, preproccess
    ...
    "get,post".replace(smcore.rword, function(method) {
        smcore[method] = function(url, data, callback, type) {
    ...
    smcore.getScript = function(url, callback) {
        return smcore.get(url, null, callback, "script")
    }
    smcore.getJSON = function(url, data, callback) {
        return smcore.get(url, data, callback, "json")
    }
    smcore.upload = function(url, form, data, callback, dataType) {
        if (typeof data === "function") {
            dataType = callback;
            callback = data;
            data = void 0;
    ...
    smcore.ajaxConverters = { //转换器，返回用户想要做的数据
        text: function(text) {
            return text || "";
        },
        ...
        html: function(text) {
            return smcore.parseHTML(text) //一个文档碎片,方便直接插入DOM树
        },
        json: function(text) {
            if (!smcore.parseJSON) {
                smcore.log("smcore.parseJSON不存在,请升级到最新版")
            }
            return smcore.parseJSON(text)
        },
        script: function(text) {
            parseJS(text)
        },
        jsonp: function() {
            var json = smcore[this.jsonpCallback];
            delete smcore[this.jsonpCallback];
            return json;
        }
    }

    smcore.param = function(json) {
        if (!smcore.isPlainObject(json)) {
            return ""
        }
        var buffer = []
    ...
    function paramInner(json, prefix, buffer) {
        ...
        for (var key in json) {
            if (json.hasOwnProperty(key)) {
                ...
                } else if (Array.isArray(val) || smcore.isPlainObject(val)) {
                    smcore.each(val, function(subKey, subVal) {
    ...
    smcore.unparam = function(url, query) {
        var json = {};
        if (!url || !smcore.type(url) === "string") {
            return json;
        }
        ...
        for (; i < len; ++i) {
            ...
            try {
                ...
                smcore.log(e + "decodeURIComponent error : " + pair[1], 3)
                val = pair[1] || "";
            }
    ...
    smcore.serialize = function(form) {
        var json = {};
        ...
        return smcore.param(json, false) // 名值键值对序列化,数组元素名字前不加 []
    }
    ...
    var transports = smcore.ajaxTransports = {
        xhr: {
            ...
            request: function() {
                ...
                smcore.log("XhrTransport.request.....")
                var transport = this.transport = new smcore.xhr;
            ...
            respond: function(event, forceAbort) {
                ...
                try {
                    ...
                    if (forceAbort || completed) {
                        transport.onreadystatechange = smcore.noop
        ...
        jsonp: {
            preproccess: function() {
                ...
                opts.url = opts.url + (rquery.test(opts.url) ? "&" : "?") + opts.jsonp + "=smcore." + name
                    //将后台返回的json保存在惰性函数中
                smcore[name] = function(json) {
                    smcore[name] = json
                }
                return "script"
            }
        ...
        script: {
            request: function() {
                ...
                smcore.log("ScriptTransport.sending.....")
                if (opts.charset) {
            ...
            respond: function(event, forceAbort) {
                ...
                if (forceAbort || execute) {
                    ...
                    if (!forceAbort) {
                        var args = typeof smcore[this.jsonpCallback] === "function" ? [500, "error"] : [200, "success"]
        ...
        upload: {
            preproccess: function() {
                ...
                smcore.each(opts.data, function(key, val) {
    ...
    smcore.mix(transports.jsonp, transports.script)
    smcore.mix(transports.upload, transports.xhr)
    ...
    if (!window.FormData) {
        ...
        smcore.fixAjax = function() {
            smcore.ajaxConverters.arraybuffer = function() {
            ...
            function createIframe(ID) {
                var iframe = smcore.parseHTML("<iframe " + " id='" + ID + "'" +
            ...
            smcore.ajaxTransports.upload = {
                request: function() {
                    ...
                    smcore.log("iframe transport...");
                    this.uploadcallback = smcore.bind(iframe, "load", function(event) {
                        self.respond(event);
                    });
                    form.submit();
                    //还原form的属性
                ...
                respond: function(event) {
                    ...
                    this.uploadcallback = smcore.unbind(node, "load", this.uploadcallback);
                    delete this.uploadcallback;
                    setTimeout(function() { // Fix busy state in FF3
                        node.parentNode.removeChild(node);
                        smcore.log("iframe.parentNode.removeChild(iframe)");
                    });
                }
            };
            delete smcore.fixAjax;
        };
        smcore.fixAjax()
    }
    return smcore
...
/**
 2011.8.31
 将会传送器的abort方法上传到smcore.XHR.abort去处理
