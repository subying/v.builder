 ...
 * <h3>使用方法</h3>
 * ```javascript
 smcore(elem).animate( properties [, duration] [, easing] [, complete] )
 smcore(elem).animate( properties, options )
 * ```
 */
define('_base/animate',['smcore'],function() {
    /*********************************************************************
     *                      主函数                                   *
     **********************************************************************/
    var effect = smcore.fn.animate = function(properties, options) {
        ...
        if (typeof properties === "number") { //如果第一个为数字
            ...
            for (var name in properties) {//处理第一个参数
                var p = smcore.cssName(name) || name
                if (name !== p) {
    ...
    function addOptions(properties) {
        ...
        this.easing = smcore.easing[this.easing] ? this.easing : "swing"//缓动公式的名字
    ...
    smcore.each(bezier, function(key, value) {
        smcore.easing[key] = bezierToEasing([value[0], value[1]], [value[2], value[3]])
    })
    ...
    /**
     * @other
     * <p>一个时间轴<code>smcore.timeline</code>中包含许多帧, 一帧里面有各种渐变动画, 渐变的轨迹是由缓动公式所规定</p>
     */
    var timeline = smcore.timeline = []
    ...
    function enterFrame(frame, index) {
        ...
        if (!frame.startTime) { //第一帧
            ...
            var elem = frame.elem
            if (smcore.css(elem, "display") === "none" && !elem.dataShow) {
    ...
    /**
     * @other
     * <p>smcore.fn.delay, smcore.fn.slideDown, smcore.fn.slideUp,
     * smcore.fn.slideToggle, smcore.fn.fadeIn, smcore.fn.fadeOut,smcore.fn.fadeToggle
     * smcore.fn.show, smcore.fn.hide, smcore.fn.toggle这些方法其实都是smcore.fn.animate的
     * 二次包装，包括<code>smcore.fn.animate</code>在内，他们的功能都是往时间轴添加一个帧对象(Frame)</p>
    ...
    smcore.isHidden = function(node) {
        return  node.sourceIndex === 0 || smcore.css(node, "display") === "none" || !smcore.contains(root, node)
    }
    ...
    Frame.prototype = {
        ...
        build: function() {
            ...
            if (elem.nodeType === 1 && ("height" in props || "width" in props)) {
                ...
                var display = style.display || smcore.css(elem, "display")
                var oldDisplay = elem.getAttribute("olddisplay")
                if (!oldDisplay) {
                    if (display === "none") {
                        style.display = ""//尝试清空行内的display
                        display = smcore.css(elem, "display")
                        if (display === "none") {
                            display = smcore.parseDisplay(elem.nodeName)
                        }
                    }
                ...
                if (display === "inline" && smcore.css(elem, "float") === "none") {
                    if (!inlineBlockNeedsLayout || smcore.parseDisplay(elem.nodeName) === "inline") {
            ...
            frame.bind("after", function() {
                if (frame.showState === "hide") {
                    ...
                    for (var i in frame.orig) { //还原为初始状态
                        ...
                        smcore.css(this, i, frame.orig[i])
                    }
                }
            })
            this.build = smcore.noop //让其无效化
        },
        createTweens: function() {
            var hidden = smcore.isHidden(this.elem)
    ...
    function createTweenImpl(frame, name, value, hidden) {
        ...
        if (/color$/.test(name)) {
            ...
            var unit = parts && parts[ 3 ] || (smcore.cssNumber[ name ] ? "" : "px")
            //处理 toggle, show, hide
            if (value === "toggle") {
            ...
            if (value === "show") {
                ...
                if (parts) {
                    ...
                    if (parts[3] && parts[ 3 ] !== unit) {//如果存在单位，并且与之前的不一样，需要转换
                        ...
                        smcore.css(clone, name, parts[2] + (parts[3] ? parts[3] : 0))
                        parts[ 2 ] = parseFloat(smcore.css(clone, name))
    ...
    function Tween(prop, options) {
        ...
        this.prop = prop
        this.easing = smcore.easing[options.easing]
    ...
    Tween.propHooks = {
        _default: {
            get: function(tween) {
                var result = smcore.css(tween.elem, tween.prop)
                return !result || result === "auto" ? 0 : result
            },
            set: function(tween) {
                smcore.css(tween.elem, tween.prop, tween.now + tween.unit)
            }
        }
    }

    smcore.each(["scrollTop", "scollLeft"], function(name) {
    ...
    smcore.each(effects, function(method, props) {
        smcore.fn[method] = function() {
            var args = [].concat.apply([props], arguments)
            return this.animate.apply(this, args)
        }
    })

    String("toggle,show,hide").replace(smcore.rword, function(name) {
        smcore.fn[name] = function() {
