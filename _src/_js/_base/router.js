 ...
 */
define('_base/router',['smcore','_base/history'],function(smcore,history) {

    function Router() {
        var table = {}
        "get,post,delete,put".replace(smcore.rword, function(name) {
            table[name] = []
        })
    ...
    Router.prototype = {
        ...
        add: function(method, path, callback, opts) {
            ...
            smcore.Array.ensure(array, this._pathToRegExp(path, opts))
        },
        ...
        /*
         *  @interface smcore.router.redirect
        ...
        /*
         *  @interface smcore.router.navigate
         *  @param hash 访问的url hash
         *  @param options 扩展配置
        ...
        */
        navigate: function(hash, options) {
            ...
            if(!smcore.state || options.silent) smcore.history && smcore.history.updateLocation(hash, smcore.mix({}, options, {silent: true}))
            // 只是写历史而已
            if(!options.silent) {
        ...
        /*
         *  @interface smcore.router.when 配置重定向规则
        ...
        */
        when: function(path, redirect) {
            ...
            smcore.each(path, function(index, p) {
        ...
        /*
         *  @interface smcore.router.get 添加一个router规则
         *  @param path url表达式
        ...
        */
        ...
        /* *
         ...
         `'/files/*path'` - ditto.
         */
        // smcore.router.get("/ddd/:dddID/",callback)
        // smcore.router.get("/ddd/{dddID}/",callback)
        // smcore.router.get("/ddd/{dddID:[0-9]{4}}/",callback)
        // smcore.router.get("/ddd/{dddID:int}/",callback)
        // 我们甚至可以在这里添加新的类型，smcore.router.$type.d4 = { pattern: '[0-9]{4}', decode: Number}
        // smcore.router.get("/ddd/{dddID:d4}/",callback)
        $types: {
            date: {
    ...
    "get,put,delete,post".replace(smcore.rword, function(method) {
    ...
    function supportLocalStorage() {
        try {
            localStorage.setItem("smcore", 1)
            localStorage.removeItem("smcore")
            return true
        } catch (e) {
            return false
        }
...
/*
 ...
 <meta charset="utf-8">
 <title>路由系统</title>
 <script src="smcore.js"></script>
 <script>
 require(["mmRouter"], function() {
 var model = smcore.define('xxx', function(vm) {
 vm.currPath = ""
 })
 smcore.router.get("/aaa", function(a) {
 model.currPath = this.path
 })
 smcore.router.get("/bbb", function(a) {
 model.currPath = this.path
 })
 smcore.router.get("/ccc", function(a) {
 model.currPath = this.path
 })
 smcore.router.get("/ddd/:ddd", function(a) {//:ddd为参数
 smcore.log(a)
 model.currPath = this.path
 })
 smcore.router.get("/eee", function(a) {
 model.currPath = this.path
 })
 smcore.history.start({
 html5Mode: true,
 basepath: "/smcore"
 })
 smcore.scan()
