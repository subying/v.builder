 ...
 * 浏览历史记录控制模块
 */
define('_base/history',['smcore'],function(smcore) {
    var anchorElement = document.createElement('a')

    var History = smcore.History = function() {
    ...
    History.prototype = {
        ...
        start: function(options) {
            if (History.started)
                throw new Error("smcore.history has already been started")
            History.started = true
            this.options = smcore.mix({}, History.defaults, options)
            ...
            if (!supportPushState) {
                if (this.html5Mode) {
                    smcore.log("如果浏览器不支持HTML5 pushState，强制使用hash hack!")
            ...
            switch (this.monitorMode) {
                case "popstate":
                    this.checkUrl = smcore.bind(window, "popstate", checkUrl)
                    this._fireLocationChange = checkUrl
                    break
                case  "hashchange":
                    this.checkUrl = smcore.bind(window, "hashchange", checkUrl)
                    break;
                case  "iframepoll":
        ...
        // 中断URL的监听
        stop: function() {
            smcore.unbind(window, "popstate", this.checkUrl)
            smcore.unbind(window, "hashchange", this.checkUrl)
    ...
    smcore.history = new History

    //https://github.com/asual/jquery-address/blob/master/src/jquery.address.js

    //劫持页面上所有点击事件，如果事件源来自链接或其内部，
    //并且它不会跳出本页，并且以"#/"或"#!/"开头，那么触发updateLocation方法
    smcore.bind(document, "click", function(event) {
        ...
        if (targetIsThisWindow(target.target)) {
            ...
            var prefix = smcore.history.prefix
            ...
            if (href.indexOf(prefix) === 0 && hash !== "") {
                ...
                smcore.router && smcore.router.navigate(hash)
            }
        }
    })
